...
def test_it__dict_from_model_object():
    ...
    group = Group(name="ravenclaw", color="blue", created_at=datetime(2000, 1, 1, 10))
    group.users = [User(name="foo", created_at=datetime(2000, 1, 1, 10))]

    dict_ = target.dict_from_object(group)
    assert dict_ == {'color': 'blue',
                     'users': [{'created_at': datetime(2000, 1, 1, 10, 0), 'pk': None, 'name': 'foo'}],
                     'created_at': datetime(2000, 1, 1, 10, 0),
                     'pk': None,
                     'name': 'ravenclaw'}
...
def test_it__jsondict_from_model():
    ...
    assert jsondict == {'color': 'blue',
                        'name': 'ravenclaw',
                        'users': [{'name': 'foo', 'pk': None, 'created_at': '2000-01-01T10:00:00Z'}],
                        'pk': None,
...
def test_it__validate__jsondict():
    ...
    group_dict = {'color': 'blue',
                  'name': 'ravenclaw',
                  'users': [{'name': 'foo', 'pk': 10, 'created_at': '2000-01-01T10:00:00Z'}],
                  'pk': 1,
                  'created_at': '2000-01-01T10:00:00Z'}

    target.validate_jsondict(group_dict)
